// bal24 v2 — 교육생 명단 요약 카드 (AI 자동 추출 + 직접 입력 + 명단 미리보기)

import { useCallback, useEffect, useRef, useState } from 'react';
import { Users, Sparkles, Upload, Pencil, Loader2 } from 'lucide-react';
...
import { PARTICIPANT_ROLE_LABEL } from '../../../../lib/participantUtils';
...
interface ExtractedPerson { name: string; email?: string | null; phone?: string | null; org?: string | null; }
...
export default function ProgramParticipantSummaryCard({ programId }: Props) {
  ...
  const reload = useCallback(async () => {
    const { data, error } = await supabase
      .from('program_participants').select('*').eq('program_id', programId)
  ...
  async function handleAiExtract() {
    ...
    try {
      ...
      const rows = persons.filter((p) => p.name?.trim()).map((p) => ({
        program_id: programId, name: p.name.trim(), role: 'participant' as const,
        email: p.email ?? null, phone: p.phone ?? null, memo: p.org ?? null,
      }));
  ...
  async function handleManualAdd() {
    ...
    try {
      const rows = lines.map((line) => {
        ...
        return {
          program_id: programId, name: name || '미지정', role: 'participant' as const,
          email: email || null, phone: phone || null, memo: org || null,
        };
      });
      const { error } = await supabase.from('program_participants').insert(rows);
      if (error) throw error;
      ...
      const r = err instanceof Error ? err.message : '';
      console.error('[overview-participant] 직접 입력 실패:', r);
    ...
    <Card>
      <CardHeader>
        <CardTitle className="text-sm font-bold text-[#1E1B4B] flex items-center gap-1.5">
          <Users size={14} className="text-violet-500" aria-hidden="true" />
          교육생 명단

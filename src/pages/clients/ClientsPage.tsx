// bal24 v2 — 고객사 목록 페이지
// 3열 카드 그리드 (기본) + 리스트 뷰 + 검색(상호명/담당자) + 신규 등록

import { useCallback, useEffect, useMemo, useState } from 'react';
import { LayoutGrid, List, Plus, Loader2, Search, Building2, Phone, Mail } from 'lucide-react';
import {
  Button,
  Card,
  CardContent,
...
function ClientGridCard({ c }: { c: Client }) {
  return (
      ...
      <CardContent className="space-y-1.5 text-xs text-muted">
        ...
        {c.manager_phone && (
          <div className="flex items-center gap-1.5">
            ...
            <span className="truncate">{c.manager_phone}</span>
          </div>
        )}
        {c.manager_email && (
          <div className="flex items-center gap-1.5">
            <Mail size={12} className="text-slate-400" />
            <span className="truncate">{c.manager_email}</span>
          </div>
        )}
      </CardContent>
    </Card>
...
function ClientListRow({ c }: { c: Client }) {
  return (
        ...
        <div className="min-w-0 text-xs text-muted truncate">
          {c.manager_name ? (
            <>
              <span className="font-semibold text-slate-700">담당</span>{' '}
              {c.manager_name}
              {c.manager_phone && ` · ${c.manager_phone}`}
            </>
          ) : (
...
export default function ClientsPage() {
  ...
  const visible = useMemo(() => {
    ...
    return clients.filter((c) => {
      const fields = [c.name, c.representative, c.manager_name].filter(Boolean) as string[];
    ...
    <div className="space-y-5 max-w-[1400px]">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {visible.map((c) => (
            <ClientGridCard key={c.id} c={c} />
          ))}
        </div>
      ) : (
        <ul className="space-y-2">
          {visible.map((c) => (
            <ClientListRow key={c.id} c={c} />
          ))}
        </ul>
      )}

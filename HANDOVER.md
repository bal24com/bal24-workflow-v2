...
## 5. 진행 중 및 미완성 기능
### 갈래 A (회계·컨소시엄 — Skywork 영역, 미확인)
- ⏳ 지급요청(PaymentRequestTab) — 특정 환경 행 삭제 시 오류 (재현 정보 필요).
- ⏳ 견적 가져오기 모달 — datalist 템플릿·prefill 강화.
- ⏳ 리포트 자동화 — 최종 결과보고서 데이터 집계 고도화.
- ⏳ 컨소시엄 상세 [+ 프로그램 추가]·[+ 태스크 추가] — 현재 **안내 toast 만**(미완성, 오류 아님).
  · 원인 — `ConProgramsTab.tsx` handleAdd / `ConTasksTab.tsx` handleAdd 가 모달 대신 toast.info 만 호출.
  · 우회 경로(정상 작동) — 프로그램 페이지 `ProgramFormModal` 에 컨소시엄 드롭다운 有(consortium_id 저장). 프로젝트 태스크 탭도 컨소시엄 선택 가능.
  · 개선안 — 두 handleAdd 에서 해당 모달을 직접 열고 consortium_id 를 prefill (모달에 prefill prop 추가).
...
## 8. 다음 세션 추천 시작점 (2026-06-03 마지막 점검 기준)
박경수님이 직접 고를 수 있게 후보를 우선순위로 정리. 새 세션은 아래 중 하나로 시작 권장.
1. **컨소시엄 모달 통합** — 컨소시엄 상세에서 [+ 프로그램/태스크 추가] 가 바로 모달을 열도록 (consortium_id prefill). 위 5번 갈래 A 마지막 항목. 작음·효과 큼.
2. **수혜기관 학교별 동아리 통합 관리** — 수혜기관 토큰에서 자기 학교 동아리 일정 확정·활동을 한 곳에서 (현재는 동아리별 토큰). 학교↔토큰 매핑 설계 필요. 큼.
3. **보고서 단계 구분** — 착수/중간(과정)/결과 보고서를 단계별로 분리 작성·노출 (현재 결과보고서 1종).
4. **지급요청 행 삭제 오류**(갈래 A) — 재현 정보(오류 메시지) 필요.

> 직전 세션(Claude)에서 점검 완료 — 컨소시엄 [+ 프로그램 추가] 는 오류 아님(의도된 미완성). 우회 경로 정상.
